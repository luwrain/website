...
В~ некоторых чертах LUWRAIN напоминает окружение [Emacspeak](http://emacspeak.sf.net),
...
1. Копирование содержимого элемента управления в буфер обмена целиком или любой его~части, скажем, чтобы послать другу.
...
1. Поиск текста в~текущем элементе управления.
...
1. Содержимое элемента управления можно последовательно прослушать  сначала или с~любой отметке в~виде речевого описания.
...
1. Фрагмент текста под~курсором может быть легко передан в~сетевой сервис с~прочтением результата обработки.
Пользователь неожиданно встретил новый термин~--- не~проблема,
комбинация клавиш Alt+G позволит осуществить запрос в~поисковую систему [DuckDuckGo](http://duckduckgo/),
...
1. Если курсор стоит на~записи некоторой денежной суммы в~иностранной валюте,
...
1. Если пользователь выделит фрагмент мелодии в~нотации [Lilypond](http://lilypond.org) или фрагмент математической записи в~нотации  [TeX](https://ru.wikipedia.org/wiki/LaTeX),
...
В~частности, в~GNU/Linux пользователь способен управлять системой при~помощи [тачпада](https://ru.wikipedia.org/wiki/%D0%A2%D0%B0%D1%87%D0%BF%D0%B0%D0%B4),
...
1. __Как  полная операционная система.__
...
Дистрибутивы в~такой форме основаны на~[Ubuntu~Linux](http://ubuntu.ru).
Это самый удобный вариант для~установки, скажем, на~небольшой ноутбук или на~любое другое компактное устройство.
1. __Как приложение для~Microsoft Windows или для~GNU/Linux.__
LUWRAIN [легко скачивается](local:/download/) в~виде дистрибутива, размером около 200 мегабайт.
...
1. __Как основа для~автономных невизуальных приложений.__
...
По~мере необходимости все~желающие люди могут дополнять функции LUWRAIN своими программами на~языке [JavaScript](https://ru.wikipedia.org/wiki/JavaScript).
Созданные программы будут выполняться движком [Nashorn](https://ru.wikipedia.org/wiki/Nashorn_%28%D0%B4%D0%B2%D0%B8%D0%B6%D0%BE%D0%BA_JavaScript%29),
...
Одной из~самых ценных возможностей является поддержка браузерного движка [WebKit](https://ru.wikipedia.org/wiki/WebKit),
...
количество которых растёт всё~быстрее и быстрее.
...
С~использованием LUWRAIN пользователь способен получать оповещения, скажем, из~[Twitter Streaming API](https://developer.twitter.com/en/docs/tweets/filter-realtime/overview.html),
